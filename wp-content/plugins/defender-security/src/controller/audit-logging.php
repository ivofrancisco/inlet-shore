...
class Audit_Logging extends Controller {
	...
	public function __construct() {
		$this->register_page(
			esc_html__( 'Audit Logging', 'defender-security' ),
			$this->slug,
			[
				&$this,
				'main_view',
	...
	public function pull_logs( Request $request ) {
		...
		if ( ! is_object( $date_from ) || ! is_object( $date_to ) ) {
			return new Response(
				false,
				[
					'message' => __( 'Invalid data', 'defender-security' ),
				]
			);
		}
	...
	*/
	...
	public function export_strings(): array {
		if ( ! ( new \WP_Defender\Behavior\WPMUDEV() )->is_pro() ) {
			return [
				sprintf( __( 'Inactive %s', 'defender-security' ), '<span class="sui-tag sui-tag-pro">Pro</span>' ),
			];
		}
