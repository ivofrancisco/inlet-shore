...
class Two_Factor extends Controller {
	...
	public function __construct() {
		$title = esc_html__( '2FA', 'defender-security' );
	...
	public function verify_otp_login_time() {
		...
		if ( empty( $user_id ) ){
			wp_die( __( 'Empty user data.', 'defender-security' ) );
		}
	...
	private function render_otp_screen( array $params = [] ) {
		...
		if ( 0 === count( $params['providers'] ) ) {
			// Since 3.5.0.
			$params['error'] = new WP_Error( 'opt_fail', 	__( 'No providers.', 'defender-security' ) );
	...
	public function data_frontend(): array {
		return array_merge(
			[
				...
				'new_feature' => '<span class="sui-tag sui-tag-beta margin-right-10">' . __( 'Beta', 'defender-security' ) . '</span>'
					. sprintf(
					/* translators: %s: link */
						__( 'Web Authentication is now available. Enable users to authenticate login using their device\'s built-in fingerprint, facial recognition, or an external hardware security key. <a target="_blank" href="%s">Learn more</a>', 'defender-security' ),
