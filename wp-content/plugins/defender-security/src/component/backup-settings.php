...
class Backup_Settings extends Component {
	...
	public function import_module_strings( array $data ): array {
		...
		foreach ( $data['configs'] as $key => $config ) {
			if (
				...
				$data['strings']['security_tweaks'][] = __( 'Email notifications active', 'defender-security' );
			} elseif ( 'scan' === $key ) {
	...
	public function create_default_module_strings( array $configs, bool $is_pro ): array {
		...
		foreach ( $configs as $key => $config ) {
			if ( 'security_tweaks' === $key ) {
				...
				$strings['two_factor'][] = __( 'Inactive', 'defender-security' );
			} elseif ( 'mask_login' === $key ) {
				$strings['mask_login'][] = __( 'Inactive', 'defender-security' );
			} elseif ( 'security_headers' === $key ) {
				$strings['security_headers'][] = __( 'Active', 'defender-security' );
			} elseif ( 'blocklist_monitor' === $key ) {
				$strings['blocklist_monitor'] = wd_di()->get( Blocklist_Monitor::class )->config_strings( $config, $is_pro );
			} elseif ( 'pwned_passwords' === $key ) {
				$strings['pwned_passwords'][] = __( 'Inactive', 'defender-security' );
			}
		}
	...
	private function module_to_name( string $module ): string {
		switch ( $module ) {
			...
			case 'blocklist_monitor':
				return __( 'Blocklist Monitor', 'defender-security' );
			case 'pwned_passwords':
	...
	private function format_blocklist_monitor_strings( array $config, bool $is_pro ): array {
		...
		if ( isset( $config['status'] ) && '1' === (string) $config['status'] ) {
			if ( $is_pro ) {
				$monitor = [ __( 'Active', 'defender-security' ) ];
			} else {
				$monitor = [
					sprintf(
						/* translators: Status of blocklist monitor */
						__( 'Active %s', 'defender-security' ),
						'<span class="sui-tag sui-tag-pro">Pro</span>'
					),
				];
			}
			...
			if ( $is_pro ) {
				$monitor = [ __( 'Inactive', 'defender-security' ) ];
			} else {
				$monitor = [
					sprintf(
						/* translators: Status of blocklist monitor */
