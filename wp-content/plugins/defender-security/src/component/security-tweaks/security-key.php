	...
	public function process() {
		if ( ! is_writable( $this->file ) ) {
			return new WP_Error(
				...
				sprintf( __( 'The file %s is not writable', 'defender-security' ), $this->file )
			);
		}
	...
	private function get_salts() {
		...
		if ( is_wp_error( $response ) ) {
			return new WP_Error(
				...
				__( 'Unable to generate salts. Please try again.', 'defender-security' )
			);
		}
	...
	public function to_array(): array {
		...
		if ( ! $this->is_salts_exist() ) {
			...
			/* translators: %s: number of days */
				__( 'Your current security keys are %s days old. Time to update them!', 'defender-security' ),
		...
		return [
			'slug' => $this->slug,
			'title' => __( 'Update old security keys', 'defender-security' ),
			'errorReason' => $error_message,
			'successReason' => sprintf(
			/* translators: %s: number of days */
				__( 'Your security keys are less than %s days old, nice work.', 'defender-security' ),
				$get_last_modified_days
			),
			'misc' => [
			...
			'bulk_title' => __( 'Security Keys', 'defender-security' ),
		];
	}
