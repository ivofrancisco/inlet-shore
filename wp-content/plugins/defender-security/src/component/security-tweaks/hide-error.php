...
class Hide_Error extends Component {
	...
	private function set_debug_data( $debug_type, $value ) {
		...
		if ( false === $obj_file ) {
			return new WP_Error(
				...
				__( 'The file wp-config.php is not writable', 'defender-security' )
			);
		...
		return $line_found
			...
			: new WP_Error(
				...
				__( 'Error writing to file.', 'defender-security' ),
				404
			);
	}
	...
	public function to_array() {
		return array(
			'slug'             => $this->slug,
			'title'            => __( 'Hide error reporting', 'defender-security' ),
			'errorReason'      => __( 'Error debugging is currently allowed.', 'defender-security' ),
			'successReason'    => __( 'You\'ve disabled all error reporting, Houston will never report a problem.', 'defender-security' ),
			'misc'             => array(),
			'bulk_description' => __( 'Error debugging feature is useful for active development, but on live sites provides hackers yet another way to find loopholes in your site\'s security. We will disable error reporting for you.', 'defender-security' ),
			'bulk_title'       => __( 'Error Reporting', 'defender-security' ),
		);
	}
}
