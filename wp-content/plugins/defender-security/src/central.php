...
class Central extends Component {
	...
	public function routing() {
		...
		if ( ! wp_verify_nonce( $nonce, $route ) ) {
			wp_send_json_error( [ 'message' => __( 'Invalid API request.', 'defender-security' ) ] );
		}
		...
		try {
			...
			if ( $is_private && ! $this->check_permission() ) {
				wp_send_json_error( [ 'message' => __( 'You shall not pass.', 'defender-security' ) ] );
			}
			if ( $is_private ) {
	...
	private function check_opcache() {
		if ( $this->is_opcache_save_comments_disabled() ) {
			wp_send_json_error(
				[
					'message' => sprintf( __( '%s is disabled. Please contact your hosting provider to enable it.', 'defender-security' ), '<strong>OPcache Save Comments</strong>' ),
				]
			);
		}
	}
