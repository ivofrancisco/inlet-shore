...
trait Defender_Hub_Client {
	...
	public function set_global_ip_list( array $data ) {
		if (
			...
			return new WP_Error( 'defender_hub_api_missing_params', __( 'Missing parameter(s)', 'defender-security' ) );
		}
		...
		if ( ! empty( $errors['allow_list'] ) || ! empty( $errors['block_list'] ) ) {
			return new WP_Error( 'defender_hub_api_invalid_ips', __( 'Invalid IP(s)', 'defender-security' ), $errors );
		} else {
			return $ret;
		}
	}
