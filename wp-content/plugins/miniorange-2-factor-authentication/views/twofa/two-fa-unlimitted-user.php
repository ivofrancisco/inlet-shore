...
if ( current_user_can( 'administrator' ) ) {
		...
		<div>
			<form name="f" method="post" action="" >
				...
				<input type="checkbox" onChange="mo_toggle_twofa()" style="padding-top: 50px;" id="mo2f_enable_2faa"
					...
					value="<?php MoWpnsUtility::get_mo2f_db_option( 'mo2f_activate_plugin', 'get_option' ); ?>"<?php checked( (int) ( MoWpnsUtility::get_mo2f_db_option( 'mo2f_activate_plugin', 'get_option' ) ) === 1 ); ?>/>
			...
			value="<?php echo esc_attr( wp_create_nonce( 'mo-two-factor-ajax-nonce' ) ); ?>"/>
				...
				<?php
				esc_attr_e( 'Enable plugin log ', 'miniorange-2-factor-authentication' );
	...
	<a class=" btn-link" data-toggle="collapse" id="showpreviewwploginpage" href="#previewwploginpage" aria-expanded="false"><?php esc_html_e( 'See preview', 'miniorange-2-factor-authentication' ); ?></a>
	<?php mo2f_setting_tooltip_array( $settings_tab_tooltip_array[1] ); ?>
		...
		<div class="mo2f_collapse" id="previewwploginpage" style="height:300px;">
					...
					<img style="height:300px;" src="<?php echo esc_url( $image_path ) . '2fa-on-login-page.png'; ?>" >
		</div>
		</div>
		<div>
			<form name="f" method="post" action="" >
				<input type="hidden" id="mo2f_nonce_enable_2FA_prompt_on_login" name="nonce"
				value="<?php echo esc_attr( wp_create_nonce( 'mo-two-factor-ajax-nonce' ) ); ?>"/>
		...
		<div>
			<form name="f" method="post" action="" >
				<input type="hidden" id="mo2f_nonce_enable_inline" name="nonce"
					value="<?php echo esc_attr( wp_create_nonce( 'mo-two-factor-ajax-nonce' ) ); ?>"/>
				<label class="mo_wpns_switch" style="float: right;">
				<input type="checkbox" onChange="mo_toggle_inline()" style="padding-top: 50px;float: right;" id="mo2f_inline_registration"
					...
					value="<?php MoWpnsUtility::get_mo2f_db_option( 'mo2f_inline_registration', 'get_option' ); ?>" <?php checked( (int) MoWpnsUtility::get_mo2f_db_option( 'mo2f_inline_registration', 'get_option' ) === 1 ); ?>/>
				<span class="mo_wpns_slider"></span>
				</label>
...
<h2>Should users be given a grace period or should they be directly enforced for 2FA setup?
	...
	<form name="f" method="post" action="">			
			...
			<div id="mo2f_grace_period_show" style="display: <?php echo( get_option( 'mo2f_grace_period' ) === 'on' ) ? 'block' : 'none'; ?>;" >
				...
				<input type="number" id="mo2f_grace_period" name= "mo2f_grace_period_value" value="<?php echo esc_attr( get_option( 'mo2f_grace_period_value' ) ); ?>" min=1 max=10>					  
		...
		<div>
			<form name="f" method="post" action="" >
				...
				<input type="checkbox" onChange="mo_toggle_configured_methods()" style="padding-top: 50px;float: right;" id="mo2f_nonce_enable_configured_methods"
					...
					value="<?php MoWpnsUtility::get_mo2f_db_option( 'mo2f_nonce_enable_configured_methods', 'site_option' ); ?>" <?php checked( MoWpnsUtility::get_mo2f_db_option( 'mo2f_nonce_enable_configured_methods', 'site_option' ) === '1' ); ?>/>
				<span class="mo_wpns_slider"></span>
				</label>
	...
	<script type="text/javascript">
		...
		function mo_toggle_twofa(){
			...
			var data =  {
				...
				'nonce'         				:  jQuery('#mo2f_nonce_enable_2FA').val(),
		...
		function mo2f_debug_log(){
			var data =  {
				...
				'nonce'                         :  jQuery('#mo2f_nonce_enable_debug_log').val(),
		...
		function mo_toggle_twofa_prompt_on_login(){
			var data =  {
				...
				'nonce'         							:  jQuery('#mo2f_nonce_enable_2FA_prompt_on_login').val(),
			...
			jQuery.post(ajaxurl, data, function(response) {
				...
				}else if(response['data'] === "false_method_onprem"){
				...
				}else if(response['data'] === 'false_method_cloud'){
		...
		function mo_toggle_inline(){
			...
			var data =  {
				...
				'nonce'                         :  jQuery('#mo2f_nonce_enable_inline').val(),
...
if ( current_user_can( 'administrator' ) ) {
	?>
		...
		<span>
						...
						echo esc_html( miniorange_2_factor_user_roles( $user ) );
						?>
						<br>
						</span>
...
if ( ! MO2F_IS_ONPREM && current_user_can( 'administrator' ) ) {
	...
	<script type="text/javascript">
		...
		jQuery('#ConfirmOnPremButton').click(function(){
			...
			jQuery.post(ajaxurl, data, function(response) {
				if(response['data'] =='OnPremiseActive')
				{
				...
				else if(response['data'] =='OnPremiseDeactive')
				{
